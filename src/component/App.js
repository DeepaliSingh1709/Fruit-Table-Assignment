...
class App extends Component {
  ...
  commonfilter=(value)=>{
    ...
    if(value){
      newdata=newdata.filter(a=>{
        var rowstring=Object.values(a).reduce((acc,x)=>""+acc+x);
        if(rowstring.indexOf(value)===-1)return false;
        else return true;
      })
    }
  ...
  render() {
    return (
      <div className={style.App}>
        <div className={style.label}><label>Common Search</label><input onChange={(e)=>{this.commonfilter(e.target.value)}}/></div>
